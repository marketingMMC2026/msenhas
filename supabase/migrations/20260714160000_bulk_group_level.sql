-- Ação em massa "adicionar ao grupo" passa a aceitar o nível: ver ou editar.
-- 'add_group' concede/mantém 'view'; 'add_group_edit' concede 'edit' (e faz
-- upgrade de um 'view' existente para 'edit'). Segue gated em can_manage_people.
...
begin
  ...
  if p_action = 'archive' then
  ...
  elsif p_action in ('add_group', 'add_group_edit') then
    v_group := p_value::uuid;
    v_level := case when p_action = 'add_group_edit' then 'edit' else 'view' end;
    insert into public.secret_permissions (secret_id, granted_to_group_id, permission_level, granted_by_id)
      select s.id, v_group, v_level, v_uid
    ...
    on conflict (secret_id, granted_to_group_id) where granted_to_group_id is not null
      do update set revoked_at = null, granted_by_id = v_uid, permission_level = excluded.permission_level;
    get diagnostics v_n = row_count;
